 ...
 */
function checkIframeConditions(document, isHidden, visibilityState, hasStatusBar) {
...
describe('detectCellType', () => {
  ...
  test('detects type from data-mode-id attribute', () => {
    const dom = new JSDOM('<div class="monaco-editor"><div data-mode-id="python"></div></div>');
  ...
  test('detects type from parent data-mode-id', () => {
    const dom = new JSDOM('<div data-mode-id="sql"><div class="monaco-editor"></div></div>');
    const editor = dom.window.document.querySelector('.monaco-editor');
...
describe('checkIframeConditions', () => {
  test('returns false when document is hidden', () => {
    ...
    expect(checkIframeConditions(dom.window.document, true, 'visible', true)).toBeFalsy();
  });
  
  test('returns false when visibility state is hidden', () => {
    const dom = new JSDOM('<body></body>');
    expect(checkIframeConditions(dom.window.document, false, 'hidden', true)).toBeFalsy();
  });
  
  test('returns false when no status bar', () => {
    const dom = new JSDOM('<body></body>');
    expect(checkIframeConditions(dom.window.document, false, 'visible', false)).toBeFalsy();
  });
  
  test('returns true when all conditions met', () => {
    const dom = new JSDOM('<body></body>');
    expect(checkIframeConditions(dom.window.document, false, 'visible', true)).toBeTruthy();
  });
});
...
describe('Status bar detection patterns', () => {
  ...
  test('detects status bar by Fluent UI classes', () => {
    ...
    const found = dom.window.document.querySelector('div.f1pha7fy.f1ewtqcl.f5ogflp');
    expect(found).toBeTruthy();
  });
  
  test('detects status bar by CellSelection button', () => {
    const dom = new JSDOM('<div><button name="CellSelection">1 of 5 cells</button></div>');
    const button = dom.window.document.querySelector('button[name="CellSelection"]');
...
describe('Cell container detection', () => {
  test('finds nteract-cell-container elements', () => {
    ...
    `);
    const cells = dom.window.document.querySelectorAll('.nteract-cell-container[data-cell-id]');
